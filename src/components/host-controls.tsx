...
export default function HostControls({ slug }: Props) {
    ...
    const togglePublishing = useCallback(async () => {
        if (isPublishing && localParticipant) {
            ...
            if (screenTrack) {
                void localParticipant.unpublishTrack(screenTrack);
            }
    ...
    }, [audioTrack, isPublishing, localParticipant, screenTrack, videoTrack]);
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <ChannelInfo streamerIdentity={slug} viewerIdentity={slug} />
        </div>
    );
}
